...
def getstable(board, color):
        ...
        if board[cind1[i]][cind2[i]] == color:
            for j in range(1,7-stop[i-1]):
                if board[cind1[i]-inc1[i-1]*j][cind2[i]-inc2[i-1]*j] != color:
                    ...
                    stable[1] += 1
    colfull = numpy.zeros((8, 8), dtype=int)
    colfull[:,numpy.sum(abs(board), axis = 0) == 8] = True
    rowfull = numpy.zeros((8, 8), dtype=int)
    rowfull[numpy.sum(abs(board), axis = 1) == 8,:] = True
    diag1full = numpy.zeros((8, 8), dtype=int)
    for i in range(15):
        diagsum = 0
        if i <= 7:
            sind1 = i
    ...
    diag2full = numpy.zeros((8, 8), dtype=int)
    for i in range(15):
        diagsum = 0
        if i <= 7:
            sind1 = i
...
def oneMove(board, myColor):
    move = onestepplace(board, myColor)
    x, y = move
    print(f"AI plays a move on ({x},{y})")
    place(board, x, y, myColor)
    print("Board is now:\n", board)
    return x, y

# 处理输入，还原棋盘
def initBoard(myColor):
    board = numpy.zeros((8, 8), dtype=int)
    board[3][4] = board[4][3] = 1 #白
    board[3][3] = board[4][4] = -1 #黑

    if myColor == -1:
        oneMove(board, myColor)
...
def main():
    ...
    options = input("输入你要执黑还是白，(输入1时执白 -- 白先黑后):\n")
    myColor = 1 if options == "1" else -1
    board = initBoard(myColor)
    print("Board is now:\n",board)

    while True:
        inputxy = input("输入你要下的位置，用空格隔开，比如1 2:\n")
        x, y = map(int, inputxy.split())
        print(f"I play a move on ({x},{y})")
        backup = numpy.copy(board)
        valid = place(board, x, y, myColor)
        while not valid:
            board = numpy.copy(backup)
            inputxy = input(f"Move is invalid ({x},{y}), please try again:\n")
            x, y = map(int, inputxy.split())
            print(f"I play a move on ({x},{y})")
            valid = place(board, x, y, myColor)
        print("Board is now:\n",board)
        x, y = oneMove(board, -myColor)
        if x < 0:
            print(f"You Win!")
            exit(0)
